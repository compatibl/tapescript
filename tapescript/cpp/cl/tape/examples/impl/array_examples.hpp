...
namespace cl
{
    ...
    inline void minus_example(std::ostream& out_str = std::cout)
    {
        ...
        // Output calculations.
        tobject diff = X[0] - X[1];
        std::vector<tobject> Y = { diff };
        out_str << "Output vector: " << Y << "\n\n";

        out_str << "Ininial Forward(0) sweep...\n\n";

# if defined CL_TAPE_ARCHIVER_TEST
        typedef tape_archive<double, boost::archive::binary_oarchive> tapewriter;
        tapewriter ss("c:\\ooo.bin");

        // Declare a tape function and stop the tape recording.
        tfunc<tvalue> f(X, Y, ss);
#   else
        // Declare a tape function and stop the tape recording.
        tfunc<tvalue> f(X, Y);
#   endif
    ...
    inline void exp_example(std::ostream& out_str = std::cout)
    {
        ...
        // Output calculations.
        tobject exp = std::exp(X[0]);
        std::vector<tobject> Y = { exp };
        out_str << "Output vector: " << Y << "\n\n";
        ...
        // Forward sweep calculations.
        std::vector<tvalue> dx = { { 2, -1 } };
        out_str << "Forward(1, dx) sweep for dx = " << dx << "..." << std::endl;
        std::vector<tvalue> forw = f.forward(1, dx, out_str);
        out_str << "Forward sweep result: " << forw << "\n\n";

        // Reverse sweep calculations.
        std::vector<tvalue> w = { { 1, -1 } };
        out_str << "Reverse(1, w) sweep for w = " << w << "..." << std::endl;
        std::vector<tvalue> rev = f.reverse(1, w, out_str);
        out_str << "Reverse sweep result: " << rev << "\n\n\n";
    }
    ...
    inline void cos_example(std::ostream& out_str = std::cout)
    {
        ...
        // Output calculations.
        tobject cos = std::cos(X[0]);
        std::vector<tobject> Y = { cos };
        out_str << "Output vector: " << Y << "\n\n";
        ...
        // Forward sweep calculations.
        std::vector<tvalue> dx = { { 1, 1 } };
        out_str << "Forward(1, dx) sweep for dx = " << dx << "..." << std::endl;
        std::vector<tvalue> forw = f.forward(1, dx, out_str);
        out_str << "Forward sweep result: " << forw << "\n\n";

        // Reverse sweep calculations.
        std::vector<tvalue> w = { { -2, 1 } };
        out_str << "Reverse(1, w) sweep for w = " << w << "..." << std::endl;
        std::vector<tvalue> rev = f.reverse(1, w, out_str);
        out_str << "Reverse sweep result: " << rev << "\n\n\n";
    }
    ...
    inline void discount_example(std::ostream& out_str = std::cout)
    {
        ...
        // Input values initialization.
        tvalue spot = { 100, 110 };
        tvalue rate = { 0.05, 0.04 };
        tvalue time = { 1, 2 };
        std::vector<tobject> X = { spot, rate, time };
        out_str << "Spot: " << spot << "\n";
        out_str << "Rate: " << rate << "\n";
        out_str << "Time: " << time << "\n";
        out_str << "Input vector: " << X << "\n";
        ...
        // Output calculations.
        tobject discount_factor = std::exp(-X[1] * X[2]);
        tobject discounted_price = X[0] * discount_factor;
        std::vector<tobject> Y = { discounted_price, discount_factor };
    ...
    inline void example1(std::ostream& out_str = std::cout)
    {
        ...
        // Output calculations.
        tobject y0 = std::sin(X[0]) / std::cos(X[1]) + X[1] / X[0];
        tobject y1 = y0 + X[0] * std::pow(X[1], 3) - 0.5;
        std::vector<tobject> Y = { y0, y1 };
        out_str << "Output vector: " << Y << "\n\n";
        ...
        // Forward sweep calculations.
        std::vector<tvalue> dx = { { 1, -1 }, 2 };
        out_str << "Forward(1, dx) sweep for dx = " << dx << "..." << std::endl;
        std::vector<tvalue> forw = f.forward(1, dx, out_str);
        out_str << "Forward sweep result: " << forw << "\n\n";

        // Reverse sweep calculations.
        std::vector<tvalue> w = { { -2, 1 }, { 2, 3 } };
        out_str << "Reverse(1, w) sweep for w = " << w << "..." << std::endl;
        std::vector<tvalue> rev = f.reverse(1, w, out_str);
        out_str << "Reverse sweep result: " << rev << "\n\n\n";
    }
    ...
    inline void sum_example(std::ostream& out_str = std::cout)
    {
        ...
        // Output calculations.
        tobject y0 = tapescript::sum_vec(X[0]);
        std::vector<tobject> Y = { y0 };
        out_str << "Output vector: " << Y << "\n\n";
        ...
        // Forward sweep calculations.
        std::vector<tvalue> dx = { { 1.5, -1 } };
        out_str << "Forward(1, dx) sweep for dx = " << dx << "..." << std::endl;
        std::vector<tvalue> forw = f.forward(1, dx, out_str);
        out_str << "Forward sweep result: " << forw << "\n\n";

        // Reverse sweep calculations.
        std::vector<tvalue> w = { 4 };
        out_str << "Reverse(1, w) sweep for w = " << w << "..." << std::endl;
        std::vector<tvalue> rev = f.reverse(1, w, out_str);
        out_str << "Reverse sweep result: " << rev << "\n\n\n";
    }
    ...
    inline void rev_example(std::ostream& out_str = std::cout)
    {
        ...
        // Output calculations.
        tobject y0 = tapescript::reverse_vec(X[0]);
        std::vector<tobject> Y = { y0 };
        out_str << "Output vector: " << Y << "\n\n";
    ...
    inline void conc_example(std::ostream& out_str = std::cout)
    {
        ...
        // Output calculations.
        tobject temp = tapescript::conc_vec(X[0], X[1]);
        tobject y0 = tapescript::conc_vec(temp, X[2]);
        std::vector<tobject> Y = { y0 };
        out_str << "Output vector: " << Y << "\n\n";
        ...
        // Forward sweep calculations.
        std::vector<tvalue> dx = { { 2, 5 }, 12, { 1, -1 } };
        out_str << "Forward(1, dx) sweep for dx = " << dx << "..." << std::endl;
        std::vector<tvalue> forw = f.forward(1, dx, out_str);
        out_str << "Forward sweep result: " << forw << "\n\n";

        // Reverse sweep calculations.
        std::vector<tvalue> w = { { 1, 2, 3, 4, 5 } };
        out_str << "Reverse(1, w) sweep for w = " << w << "..." << std::endl;
        std::vector<tvalue> rev = f.reverse(1, w, out_str);
        out_str << "Reverse sweep result: " << rev << "\n\n\n";
    }
    ...
    inline void make_example(std::ostream& out_str = std::cout)
    {
        ...
        // Output calculations.
        tobject y0 = tapescript::make_vec(X[0], 2);
        std::vector<tobject> Y = { y0 };
        out_str << "Output vector: " << Y << "\n\n";
        ...
        // Forward sweep calculations.
        std::vector<tvalue> dx = { 1 };
        out_str << "Forward(1, dx) sweep for dx = " << dx << "..." << std::endl;
        std::vector<tvalue> forw = f.forward(1, dx, out_str);
        out_str << "Forward sweep result: " << forw << "\n\n";

        // Reverse sweep calculations.
        std::vector<tvalue> w = { { 1, 2 } };
        out_str << "Reverse(1, w) sweep for w = " << w << "..." << std::endl;
        std::vector<tvalue> rev = f.reverse(1, w, out_str);
        out_str << "Reverse sweep result: " << rev << "\n\n\n";
    }
    ...
    inline void max_example(std::ostream& out_str = std::cout)
    {
        ...
        // Forward sweep calculations.
        std::vector<tvalue> dx = { { 1, 1, 1 }, { 2, 2, 2 } };
        out_str << "Forward(1, dx) sweep for dx = " << dx << "..." << std::endl;
        std::vector<tvalue> forw = f.forward(1, dx, out_str);
        out_str << "Forward sweep result: " << forw << "\n\n";

        // Reverse sweep calculations.
        std::vector<tvalue> w = { { 1, 2, 3 } };
        out_str << "Reverse(1, w) sweep for w = " << w << "..." << std::endl;
        std::vector<tvalue> rev = f.reverse(1, w, out_str);
        out_str << "Reverse sweep result: " << rev << "\n\n\n";
    }
    ...
    inline void norm_example(std::ostream& out_str = std::cout)
    {
        ...
        // Output calculations.
        tobject array = tapescript::make_vec(X[0], 2);
        tobject norm = std::sqrt(tapescript::sum_vec(std::pow(array, 2)));
        std::vector<tobject> Y = { norm };
        out_str << "Output vector: " << Y << "\n\n";
    ...
    inline void linear_regression_example(std::ostream& out_str = std::cout)
    {
        ...
        tobject x_centralized = x - x_mean;
        tobject y_centralized = y - y_mean;
        // Variance times n: n * Var[x]
        tobject var_x_n = tapescript::sum_vec(x_centralized * x_centralized);
        // Covariance times n: n * Cov[x, y]
        tobject cov_xy_n = tapescript::sum_vec(x_centralized * y_centralized);
        ...
        // Forward sweep calculations.
        std::vector<tvalue> dx = { { 1, 0, 0 }, { 0, 0, 0 } };
        out_str << "Forward(1, dx) sweep for dx = " << dx << "..." << std::endl;
        std::vector<tvalue> forw = f.forward(1, dx, out_str);
        out_str << "Forward sweep result: " << forw << "\n\n";

        // Reverse sweep calculations.
        std::vector<tvalue> w = { 0, 1, 0 };
        out_str << "Reverse(1, w) sweep for w = " << w << "..." << std::endl;
        std::vector<tvalue> rev = f.reverse(1, w, out_str);
        out_str << "Reverse sweep result: " << rev << "\n\n\n";
    }
